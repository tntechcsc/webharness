  ...
  const cpuData = useMemo(() => {
    ...
    Object.entries(dataHistory).forEach(([procName, points]) => {
      points.forEach((dp, idx) => {
        if (!chartData[idx]) {
          chartData[idx] = { time: new Date(dp.timestamp).toLocaleTimeString() };
        }
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <Typography variant="h6" sx={{ mb: 1 }}>
        Resource Utilization {selectedApp ? `for ${selectedApp}` : "for All Processes"}
      </Typography>
...
const HomePage = () => {
  ...
  return (
      ...
      <Box sx={{ flexGrow: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <Container sx={{ mt: 5, ml: 2, width: '100%', overflowX: 'auto' }} maxWidth={false}>
